...
class FriendService {
  ...
  static archiveBlockFriend = async (friend) => {
    ...
    const newStatus =
      user_id === friendExist.dataValues.friend1_id
        ? getNewStatus("FRIEND1", "FRIEND2", friendExist[statusField])
        : getNewStatus("FRIEND2", "FRIEND1", friendExist[statusField]);
  ...
  static updateExpense = async (updatedExpenseData, conversation_id) => {
    ...
    if (
      !(
        (updatedExpenseData.payer_id === (friendExist.friend1_id || friendExist.friend2_id))
        &&
        (updatedExpenseData.debtor_id === (friendExist.friend1_id || friendExist.friend2_id))
      )
    ) {
